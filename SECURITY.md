...
| # | Risk | Status |
|---|---|---|
| R1 | `/copilot/extractions` and `/copilot/lab-trend` rely on OpenEMR session ACL alone — no per-patient care-relationship check before the query. | **Resolved by design — 2026-05-05.** Re-reading the W1 architecture: "*the agent backend validates the physician's OpenEMR session… it inherits the session user's identity*." OpenEMR's own ACL is the authoritative authorization gate; the agent intentionally does not parallel it. The current posture — `ALLOWED_IFRAME_ORIGINS`-tightened CORS + same-origin browser context + OpenEMR session for the iframe — is the documented design, not a half-fix. A separate per-patient relationship check on the agent would be more restrictive than OpenEMR itself (which doesn't have native per-patient care-relationship ACLs; access is role + facility based). If a tenant policy ever requires per-patient relationships, the right place to add it is OpenEMR's ACL system, not a parallel one on the agent. |
